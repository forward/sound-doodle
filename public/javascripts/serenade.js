...
(function() {
    ...
    Midi.prototype.play = function(loop, cb) {
	...
	for(var trackName in this.tracks) {
		...
		var seq1,seq;
		if(loop === true) {
		    seq1 = [new PSequence(that.tracks[tn].notes,Infinity)];
		    seq2 = new PSequence(that.tracks[tn].durations,Infinity);
		} else {
		    seq1 = [new PSequence(that.tracks[tn].notes)];
		    seq2 = new PSequence(that.tracks[tn].durations);
		}

		audiolet.scheduler.play(seq1,seq2,
					function(notation) {					    
					    if(cb != null)
						cb(tn,notation);
